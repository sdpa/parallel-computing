...
//
    ...
    thread_arg_t *targs = (thread_arg_t *) malloc(num_threads * sizeof(thread_arg_t));
    if (!threads || !targs) {
        fprintf(stderr, "Allocation of threads failed\n");
        exit(1);
    }
...
// VS: ... parallel merge sort
    ...
    for (j = 0; j < num_threads; j++) {
        targs[j].thread_id = j;
        targs[j].q = q;
        if (pthread_create(&threads[j], NULL, thread_routine, &targs[j]) != 0) {
...
// VS: ... destroy mutex, condition variables, etc.
    ...
    free(threads); free(targs); free(ptr);
